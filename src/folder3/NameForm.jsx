...
function NameForm(props) {
    // 변수 value 를 state 로 설정, 기본값은 빈 문자열
    ...
    const handleChange = (event) => {
        // state 로 설정된 변수 value 의 값을 변경하는 setValue() 함수 실행
        // event 객체를 통해서 이벤트가 발생한 대상에 대한 정보를 가져옴
        // 이벤트 발생 대상이 가지고 있는 value 값을 사용하여 state 를 변경
    ...
    return (
        <form onSubmit={handleSubmit} className={"container"}>
            <div className={"my-3"}>
                ...
                {/* input 태그의 value 를 state 로 설정된 변수를 사용하여 화면에 데이터를 츨력 */}
                {/* 사용자 입력에 의해서 값이 변경될 경우 이벤트를 발생(onChange) */}
